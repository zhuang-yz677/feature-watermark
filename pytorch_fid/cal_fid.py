...
def get_activations(
    ...
    # images: List[PIL.Image] => Tensor[Tensor[torch.float32]]
    ...
    for batch in images:
        t_imgs.append(trans(batch))

    t_imgs = torch.stack(t_imgs, dim=0).to(device)
    pred = model(t_imgs)[0]
...
def compute_statistics_of_generate_image(images, model, batch_size, dims, device, num_workers=1):
...
def calculate_fid(paths, fid_model, device, batch_size=25, dims=2048, num_workers=1):
    ...
    if paths[0].endswith(".npz"):
        ...
        m1, s1 = compute_statistics_of_generate_image(
            paths[0], fid_model, batch_size, dims, device, num_workers
        )

    m2, s2 = compute_statistics_of_generate_image(
        paths[1], fid_model, batch_size, dims, device, num_workers
    )
